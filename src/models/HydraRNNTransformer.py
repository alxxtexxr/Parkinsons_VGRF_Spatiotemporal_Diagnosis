...
# sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../tsai')))
...
class _HydraRNNTransformer_Base(torch.nn.Module):
    def __init__(self,
                 # RNNTransformer  
                 c_in, c_out, seq_len, hidden_size=128, rnn_layers=1, bias=True, rnn_dropout=0, bidirectional=False,
                 encoder_layers=3, s=3, n_heads:int=16, d_k=None, d_v=None, 
        ...
        # RNN
        self.rnn = self._cell(k, hidden_size, num_layers=rnn_layers, bias=bias, batch_first=True, dropout=rnn_dropout, 
    ...
    def forward(self, x):
        # Hydra
        hydra_output = self.hydra_backbone(x) # [B, L, D]
        
        # RNN
        rnn_input = hydra_output
        rnn_output, _ = self.rnn(rnn_input) # output from all sequence steps: [bs x q_len x hidden_size * (1 + bidirectional)]
        
        # Transformer
        z = self.encoder(rnn_output)      # z: [bs x q_len x d_model]
        z = z.transpose(2,1).contiguous() # z: [bs x d_model x q_len]
        z = self.encoder_head(z)
        return z

class HydraRNNTransformer(_HydraRNNTransformer_Base):
    _cell = torch.nn.RNN
    
class HydraLSTMTransformer(_HydraRNNTransformer_Base):
    _cell = torch.nn.LSTM